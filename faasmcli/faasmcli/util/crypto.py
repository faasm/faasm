...
def get_random_string(length):
    ...
    return result_str
def encrypt_aes_gcm_128(msg, encryption_key, nonce_size=NONCE_SIZE, key_size=AES_KEY_SIZE):
...
def decrypt_aes_gcm_128(ciphertext, nonce, tag, decryption_key, nonce_size=NONCE_SIZE, key_size=AES_KEY_SIZE):
    aesCipher = AES.new(decryption_key[:key_size], AES.MODE_GCM, nonce=nonce)
    return aesCipher.decrypt_and_verify(ciphertext, tag)
def encrypt_file(module_path, output_path, nonce_size=NONCE_SIZE, key_size=AES_KEY_SIZE) -> (str, str, str): #encryptes given file with module path with nonce and tag
    ...
    f.close()
    return key
def get_file_hash_digest(file_path, offset=NONCE_SIZE + TAG_SIZE) -> str: #generates sha256 for file
    BLOCK_SIZE = 64*1024
    """
