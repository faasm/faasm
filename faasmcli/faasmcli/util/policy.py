        ...
        while len(stack[current_index:]) > 0:
            ...
            if meta_char == '<' and current_index == 0:
                ...
                for next_id in next_function:
                    if verify_objects[next_id] == 'end: {}:{}'.format(name, id):
                        _found = True
                        break
                if not _found:
                    return False
                ...
                test = list(reversed(self._verify_chain[last_writer_func][str(int(last_writer_id))]['nodes']))
                for _index, _label in enumerate(list(reversed(self._verify_chain[last_writer_func][str(int(last_writer_id))]['nodes']))):
                    if _label == 'write:{}'.format(state_key):
                        last = len(self._verify_chain[last_writer_func][str(int(last_writer_id))]['nodes']) - 1 - _index
                        break
                cur = 0
                internal = True
                while cur < len(reversed_execution_stack):
                    ...
                    if char == ">":
                        _count = 0
                        for _index, _char in enumerate(reversed_execution_stack[cur:]):
                            if _char == ">":
                                if _count == 0:
                                    _start = _index
                                _count += 1
                        ...
                        _found = False
                        for _id, _label in enumerate(list(reversed(self._verify_chain[last_writer_func][str(int(last_writer_id))]['nodes'][:int(last)]))):
                        ...
                        if not _found:
                            return False
                        _split = new.split(':')
                        if self._verify_chain[last_writer_func][str(int(last_writer_id))]['nodes'][last] != '{}:{}'.format(_split[0], _split[2]):
                            return False
                        _split = new.split(':')
                        new = '<{}>'.format(new)
                        if not self._is_valid_execution(new, _split[0], _split[2]):
                            return False
                        internal = True
                    elif char == ",":
                        cur += 1
                        ...
                        for _index, _char in enumerate(reversed_execution_stack[cur:]):
                            ...
                            if _char == ',':
                                ...
                                cur += _index
                                break
                        new = reversed_execution_stack[tmp-1:cur][::-1][:-1]
                        if internal:
                            _found = False
                            for _id, a in enumerate(list(reversed(self._verify_chain[last_writer_func][str(int(last_writer_id))]['nodes'][:int(last)]))):
                            ...
                            if tmp_internal:
                                ...
                                if self._verify_chain[last_writer_func][str(int(last_writer_id))]['nodes'][last] != 'start: {}:{}'.format(_split[0], _split[2]):
                                    return False
                        else:
                            _split = new.split(":")
                            _found = False
                            for _id, _label in enumerate(list(reversed(self._verify_chain[_split[0]][str(int(_split[2]))]['nodes']))):
                                if not _label.startswith('write'):
                                    if _label == '{}:{}'.format(last_writer_func, last_writer_id):
                                        last_writer_func = _split[0]
                                        last_writer_id = _split[2]
                                        last = len(self._verify_chain[last_writer_func][str(int(last_writer_id))]['nodes']) - 1 - _id
                                        _found = True
                                        break
                            if not _found:
                                return False
