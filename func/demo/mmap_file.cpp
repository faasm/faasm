...
FAASM_MAIN_FUNC() {
    ...
    if (strncmp(expected, mmappedPtr, checkLength) != 0) {
        printf("Expected and actual don't match (%s != %s)\n", expected, mmappedPtr);
        return 1;
    }

    printf("Expected file and actual file match (%s == %s)\n", expected, mmappedPtr);
    return 0;
}