...
void stackSizeCheck()
{
    // Want to create a big stack but not too big. We expect at least 1MB to be
    // available.
    ...
    // Sense check order of the stack variables. The main stack should grow
    // downwards but the contents of the arrays should go upwards in order
    ...
    if (diff < expected) {
        printf("Unexpectedly small amount of stack provisioned. Expected %li "
               ">= %li\n",
               diff,
               expected);
        exit(1);
    ...
    if (diff < minimumStack) {
        printf("Provisioned stack is smaller than the minimum required stack "
               "of %li\n",
               minimumStack);
        exit(1);
    }
    ...
    printf(
      "Stack space successfully provisioned %li (aimed for at least %li)\n",
      diff,
      expected);
