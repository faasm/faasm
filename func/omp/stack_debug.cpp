...
FAASM_MAIN_FUNC() {
    // The end result should be each of the thread numbers written into the relevant slots
    ...
    {
        // Keep the thread num in a variable that should be untouched by other threads
        int threadNum = omp_get_thread_num();

        // These two stack variables will have the same offsets for every thread,
        // hence passing them out to the runtime to be written will fail if the mapping
