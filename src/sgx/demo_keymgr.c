...
void* handle_message(void* args)
{
    ...
    while (recv(client_socket, (void*)&recv_msg, sizeof(sgx_wamr_msg_t), 0) >
           0) {
        tmp = calloc(sizeof(sgx_wamr_msg_t) + recv_msg.payload_len,
                     sizeof(uint8_t));
        memcpy(tmp, &recv_msg, sizeof(sgx_wamr_msg_t));
        recv(client_socket,
             (uint8_t*)tmp + sizeof(sgx_wamr_msg_t),
             recv_msg.payload_len,
             0);
        ...
        printf("Msg_id: %d Payload_len: %d Payload: %s Msg_len: %d\n",
               ...
               tmp->payload_len,
               tmp->payload,
               sizeof(sgx_wamr_msg_t) + tmp->payload_len);
...
int main(int argc, char** argv)
{
    ...
    while ((new_socket = accept(server_socket,
        ...
        if ((pthread_create(
              &message_thread, NULL, handle_message, (void*)new_socket))) {
