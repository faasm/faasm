...
extern "C"
{
                                           ...
                                           unsigned int output_size)
    {
        ...
        if ((output_data = (sgx_wamr_encrypted_data_blob_t *) calloc(output_data_len, sizeof(uint8_t))) == NULL) {
        ...
        read_lock(&_rwlock_faasm_sgx_tcs_realloc);
        if (sgx_rijndael128GCM_encrypt(&faasm_sgx_tcs[tls_thread_id]->env.payload_secret, output, output_size, output_data->data, p_iv, SGX_AESGCM_IV_SIZE, NULL, 0, &output_data->tag) != SGX_SUCCESS) {
            read_unlock(&_rwlock_faasm_sgx_tcs_realloc);
            free(output_data);
        ...
        memcpy(faasm_sgx_tcs[tls_thread_id]->env.output_nonce, p_iv, sizeof(output_data->nonce));
        output_data->data_len = output_size;
        read_unlock(&_rwlock_faasm_sgx_tcs_realloc);
        if((sgxReturnValue = ocall_faasm_write_output((uint8_t *) output_data, output_data_len)) != SGX_SUCCESS){
            free(output_data);
                                             ...
                                             const uint32_t buffer_len)
    {
        ...
        read_lock(&_rwlock_faasm_sgx_tcs_realloc);
        msg_payload_len = sizeof(sgx_wamr_msg_state_read_t) + strlen(faasm_sgx_tcs[tls_thread_id]->env.user) + strlen(":") + strlen(key);
        if ((msg_payload = (sgx_wamr_msg_state_read_t *) calloc(msg_payload_len, sizeof(uint8_t *))) == NULL) {
            read_unlock(&_rwlock_faasm_sgx_tcs_realloc);
            SET_ERROR(FAASM_SGX_OUT_OF_MEMORY);
            return 1;
        }
        memcpy(msg_payload->state_name, faasm_sgx_tcs[tls_thread_id]->env.user, strlen(faasm_sgx_tcs[tls_thread_id]->env.user));
        ...
        if ((encrypted_state = (sgx_wamr_encrypted_state_blob_t*) calloc(encrypted_state_len, sizeof(uint8_t))) == NULL) {
            free(res_payload);
            SET_ERROR(FAASM_SGX_OUT_OF_MEMORY);
            return 1;
        }
        if((sgxReturnValue = ocall_faasm_read_state(&returnValue, key, (uint8_t *) encrypted_state, encrypted_state_len)) != SGX_SUCCESS){
            free(res_payload);
        ...
        if (memcmp(typed_res_payload->buffer_nonce, encrypted_state->nonce, SGX_AESGCM_IV_SIZE) != 0) {
            free(res_payload);
            free(encrypted_state);
            SET_ERROR(FAASM_SGX_INVALID_INPUT_NONCE);
            return 1;
        }
        if ((execution_stack = (char *) calloc(res_payload_len - sizeof(sgx_wamr_msg_state_read_res_t) + 1 + strlen(key) + strlen(":[]"), sizeof(uint8_t))) == NULL) {
            free(res_payload);
            free(encrypted_state);
            SET_ERROR(FAASM_SGX_OUT_OF_MEMORY);
            return 1;
        ...
        memcpy(execution_stack + strlen("[;") + strlen(key), typed_res_payload->data, res_payload_len - sizeof(sgx_wamr_msg_state_read_res_t));
        memcpy(execution_stack + strlen("[;") + strlen(key) + res_payload_len - sizeof(sgx_wamr_msg_state_read_res_t), "]", strlen("]"));
        read_lock(&_rwlock_faasm_sgx_tcs_realloc);
        if ((faasm_ret_val = push_to_execution_stack(&faasm_sgx_tcs[tls_thread_id]->env.execution_stack, execution_stack)) != FAASM_SGX_SUCCESS) {
            read_unlock(&_rwlock_faasm_sgx_tcs_realloc);
            free(res_payload);
            free(encrypted_state);
        ...
        free(execution_stack);
        if (sgx_rijndael128GCM_decrypt(&typed_res_payload->state_secret, encrypted_state->data, buffer_len, buffer_ptr, encrypted_state->nonce, SGX_AESGCM_IV_SIZE, NULL, 0, &encrypted_state->tag) != SGX_SUCCESS) {
        free(res_payload);
            free(encrypted_state);
        ...
        free(res_payload);
        free(encrypted_state);
        if (sgx_sha256_msg(buffer_ptr, buffer_len, &buffer_hash) != SGX_SUCCESS) {
                                           ...
                                           const uint32_t buffer_len)
    {
        ...
        read_lock(&_rwlock_faasm_sgx_tcs_realloc);
        if ((faasm_ret_val = create_total_execution_stack(&total_execution_stack, faasm_sgx_tcs[tls_thread_id]->env.entry_execution_stack, faasm_sgx_tcs[tls_thread_id]->env.execution_stack)) != FAASM_SGX_SUCCESS) {
            read_unlock(&_rwlock_faasm_sgx_tcs_realloc);
            SET_ERROR(faasm_ret_val);
            return;
        }
        domain_len = strlen(faasm_sgx_tcs[tls_thread_id]->env.user) + strlen(":") + strlen(key);
        msg_payload_len = sizeof(sgx_wamr_msg_state_write_t) + domain_len + strlen(total_execution_stack);
        if ((msg_payload = (sgx_wamr_msg_state_write_t*) calloc(msg_payload_len, sizeof(uint8_t*))) == NULL) {
            read_unlock(&_rwlock_faasm_sgx_tcs_realloc);
        ...
        memcpy(msg_payload->buffer_nonce, p_iv, sizeof(msg_payload->buffer_nonce));
        msg_payload->name_length = domain_len;
        memcpy(msg_payload->data, faasm_sgx_tcs[tls_thread_id]->env.user, strlen(faasm_sgx_tcs[tls_thread_id]->env.user));
        memcpy(msg_payload->data + strlen(faasm_sgx_tcs[tls_thread_id]->env.user), ":", strlen(":"));
        memcpy(msg_payload->data + strlen(faasm_sgx_tcs[tls_thread_id]->env.user) + strlen(":"), key, strlen(key));
        read_unlock(&_rwlock_faasm_sgx_tcs_realloc);
        ...
        if (sgx_rijndael128GCM_encrypt(&typed_res_payload->payload_key, (uint8_t*) buffer_ptr, buffer_len, encrypted_state->data, p_iv, SGX_AESGCM_IV_SIZE, NULL, 0, &encrypted_state->tag) != SGX_SUCCESS) {
        free(res_payload);
            free(encrypted_state);
            if((sgxReturnValue = ocall_faasm_unlock_state_write2(key, encrypted_state_len)) != SGX_SUCCESS){
                SET_ERROR(FAASM_SGX_OCALL_ERROR(sgxReturnValue));
    ...
    free(res_payload);
        ...
        if((sgxReturnValue = ocall_faasm_write_state(key, (uint8_t *)encrypted_state, encrypted_state_len)) != SGX_SUCCESS){
            free(encrypted_state);
            if((sgxReturnValue = ocall_faasm_unlock_state_write2(key, encrypted_state_len)) != SGX_SUCCESS){
                SET_ERROR(FAASM_SGX_OCALL_ERROR(sgxReturnValue));
                return;
        ...
        read_lock(&_rwlock_faasm_sgx_tcs_realloc);
        msg_payload_len = sizeof(sgx_wamr_msg_state_read_t) + strlen(faasm_sgx_tcs[tls_thread_id]->env.user) + strlen(":") + strlen(key);
        if ((msg_read_payload = (sgx_wamr_msg_state_read_t *) calloc(msg_payload_len, sizeof(uint8_t *))) == NULL) {
            read_unlock(&_rwlock_faasm_sgx_tcs_realloc);
            if((sgxReturnValue = ocall_faasm_unlock_state_write2(key, encrypted_state_len)) != SGX_SUCCESS){
                SET_ERROR(FAASM_SGX_OCALL_ERROR(sgxReturnValue));
                return;
        ...
        memcpy(msg_read_payload->state_name, faasm_sgx_tcs[tls_thread_id]->env.user, strlen(faasm_sgx_tcs[tls_thread_id]->env.user));
                                                 ...
                                                 unsigned int input_size)
    {
        ...
        if ((encrypted_input = (sgx_wamr_encrypted_data_blob_t *) calloc(input_size + sizeof(sgx_wamr_encrypted_data_blob_t), sizeof(uint8_t))) == NULL) {
            SET_ERROR(FAASM_SGX_OUT_OF_MEMORY);
            return 1;
        }
        if (sgx_rijndael128GCM_encrypt(&master_secret, input, input_size, encrypted_input->data, p_iv1, SGX_AESGCM_IV_SIZE, NULL, 0, &encrypted_input->tag) != SGX_SUCCESS) {
        ...
        read_lock(&_rwlock_faasm_sgx_tcs_realloc);
        if((it = faasm_sgx_tcs[tls_thread_id]->env.chain_policy->find(name)) == faasm_sgx_tcs[tls_thread_id]->env.chain_policy->end()) {
            read_unlock(&_rwlock_faasm_sgx_tcs_realloc);
            free(encrypted_input);
        ...
        read_unlock(&_rwlock_faasm_sgx_tcs_realloc);
        policy_len = sizeof(sgx_wamr_execution_policy_t) + strlen(total_execution_stack) + 1;
        if ((policy = (sgx_wamr_execution_policy_t *) calloc(policy_len, sizeof(uint8_t))) == NULL) {
            free(encrypted_input);
            free_execution_stack(&total_execution_stack);
            SET_ERROR(FAASM_SGX_OUT_OF_MEMORY);
            return 1;
        ...
        memcpy(policy->data, total_execution_stack, strlen(total_execution_stack));
        free_execution_stack(&total_execution_stack);
        if (sgx_read_rand(p_iv, SGX_AESGCM_IV_SIZE) != SGX_SUCCESS) {
            free(encrypted_input);
            free(policy);
        ...
        encrypted_policy_len = sizeof(sgx_wamr_encrypted_data_blob_t) + policy_len;
        if ((encrypted_policy = (sgx_wamr_encrypted_data_blob_t *) calloc(encrypted_policy_len, sizeof(uint8_t))) == NULL) {
            free(encrypted_input);
            free(policy);
            SET_ERROR(FAASM_SGX_OUT_OF_MEMORY);
            return 1;
        }
        if (sgx_rijndael128GCM_encrypt(&master_secret, (uint8_t *) policy, policy_len, (uint8_t*) encrypted_policy->data, p_iv, SGX_AESGCM_IV_SIZE, NULL, 0, &encrypted_policy->tag) != SGX_SUCCESS) {
            free(encrypted_input);
            free(policy);
        ...
        if((sgxReturnValue = ocall_faasm_chain_name(&returnValue, name, (uint8_t *) encrypted_input, input_size + sizeof(sgx_wamr_encrypted_data_blob_t), (uint8_t*) encrypted_policy, encrypted_policy_len)) != SGX_SUCCESS) {
            free(encrypted_policy);
            free(encrypted_input);
            SET_ERROR(FAASM_SGX_OCALL_ERROR(sgxReturnValue));
            return 1;
                                                ...
                                                unsigned int input_size)
    {
        ...
        if (sgx_rijndael128GCM_encrypt(&master_secret, input_data, input_size, encrypted_input->data, p_iv1, SGX_AESGCM_IV_SIZE, NULL, 0, &encrypted_input->tag) != SGX_SUCCESS) {
            free(encrypted_input);
            SET_ERROR(FAASM_SGX_ENCRYPTION_FAILED);
            return 1;
        }
        ...
        memcpy(policy->input_nonce, p_iv1, sizeof(policy->input_nonce));
        memcpy(policy->hash, faasm_sgx_tcs[tls_thread_id]->env.op_code_hash, sizeof(policy->hash));
        memcpy(policy->data, total_execution_stack, strlen(total_execution_stack));
        free_execution_stack(&total_execution_stack);
        read_unlock(&_rwlock_faasm_sgx_tcs_realloc);
        ...
        encrypted_policy->data_len = policy_len;
        memcpy(encrypted_policy->nonce, p_iv, sizeof(encrypted_policy->nonce));
        if((sgxReturnValue = ocall_faasm_chain_ptr(&returnValue, wasmFuncPtr, (uint8_t *) encrypted_input, sizeof(sgx_wamr_encrypted_data_blob_t) + input_size, (uint8_t*) encrypted_policy, encrypted_policy_len)) != SGX_SUCCESS) {
            free(encrypted_policy);
            free(encrypted_input);
            SET_ERROR(FAASM_SGX_OCALL_ERROR(sgxReturnValue));
            return 1;
                                                 ...
                                                 unsigned int call_id)
    {
        ...
        read_lock(&_rwlock_faasm_sgx_tcs_realloc);
        if((it = faasm_sgx_tcs[tls_thread_id]->env.nonce_store->find(call_id)) == faasm_sgx_tcs[tls_thread_id]->env.nonce_store->end()) {
            read_unlock(&_rwlock_faasm_sgx_tcs_realloc);
        ...
        uint8_t* raw_msg = (uint8_t*) faasm_sgx_tcs[tls_thread_id]->result_ptr->buffer_ptr;
        uint32_t msg_len = faasm_sgx_tcs[tls_thread_id]->result_ptr->buffer_len;
        if (msg_len < sizeof(sgx_wamr_encrypted_data_blob_t) + sizeof(sgx_wamr_execution_attestation_t)) {
            SET_ERROR(FAASM_SGX_MISSING_RESULT);
            return 1;
        }
        sgx_wamr_encrypted_data_blob_t *msg = (sgx_wamr_encrypted_data_blob_t*) raw_msg;
        if ((decrypted_result = (sgx_wamr_execution_attestation_t *) calloc(msg->data_len, sizeof(uint8_t))) == NULL) {
            SET_ERROR(FAASM_SGX_OUT_OF_MEMORY);
            return 1;
        }
        if (sgx_rijndael128GCM_decrypt((sgx_aes_gcm_128bit_key_t*) master_secret, msg->data, msg->data_len, (uint8_t*)decrypted_result, msg->nonce, SGX_AESGCM_IV_SIZE, NULL, 0, (const sgx_aes_gcm_128bit_tag_t*)msg->tag) != SGX_SUCCESS) {
            free(decrypted_result);
            SET_ERROR(FAASM_SGX_DECRYPTION_FAILED);
            return 1;
        }
        if (memcmp(it->second.c_str(), decrypted_result->policy_nonce, SGX_AESGCM_IV_SIZE) != 0) {
            free(decrypted_result);
      ...
      unsigned int buffer_size)
    {
        ...
        if ((sgxReturnValue = ocall_faasm_await_call_output(&returnValue, call_id)) != SGX_SUCCESS) {
            SET_ERROR(FAASM_SGX_OCALL_ERROR(sgxReturnValue));
            return 1;
        }
        sgx_wamr_encrypted_data_blob_t *output = (sgx_wamr_encrypted_data_blob_t*) faasm_sgx_tcs[tls_thread_id]->output_ptr->buffer_ptr;
        uint8_t *result = (uint8_t*) faasm_sgx_tcs[tls_thread_id]->result_ptr->buffer_ptr, *decrypted_output;
        uint32_t result_len = faasm_sgx_tcs[tls_thread_id]->result_ptr->buffer_len, output_len = faasm_sgx_tcs[tls_thread_id]->output_ptr->buffer_len;
        sgx_wamr_encrypted_data_blob_t *msg = (sgx_wamr_encrypted_data_blob_t *) result;
        if (result_len <= sizeof(sgx_wamr_encrypted_data_blob_t) + sizeof(sgx_wamr_execution_attestation_t)) {
            SET_ERROR(FAASM_SGX_MISSING_RESULT);
            return 1;
        }
        if ((decrypted_result = (sgx_wamr_execution_attestation_t *) calloc(msg->data_len, sizeof(uint8_t))) == NULL) {
        ...
        if (sgx_rijndael128GCM_decrypt(&master_secret, msg->data, msg->data_len, (uint8_t *)decrypted_result, msg->nonce, SGX_AESGCM_IV_SIZE, NULL, 0, &msg->tag) != SGX_SUCCESS) {
            free(decrypted_result);
            SET_ERROR(FAASM_SGX_DECRYPTION_FAILED);
            return 1;
        }
        ...
        if (memcmp(output->nonce, decrypted_result->output_nonce, sizeof(decrypted_result->output_nonce)) != 0) {
            SET_ERROR(FAASM_SGX_RESULT_OUTPUT_MISLINK);
            return 1;
        }
        read_lock(&_rwlock_faasm_sgx_tcs_realloc);
        if ((result_stack = (char*) calloc(decrypted_result->execution_stack_len + 1, sizeof(uint8_t))) == NULL) {
            free(decrypted_result);
            SET_ERROR(FAASM_SGX_OUT_OF_MEMORY);
            return 1;
        }
        memcpy(result_stack, decrypted_result->data, decrypted_result->execution_stack_len);
        if ((faasm_ret_val = push_to_execution_stack(&faasm_sgx_tcs[tls_thread_id]->env.execution_stack, result_stack)) != FAASM_SGX_SUCCESS) {
            read_unlock(&_rwlock_faasm_sgx_tcs_realloc);
            free(decrypted_result);
            free(result_stack);
            SET_ERROR(faasm_ret_val);
        ...
        read_unlock(&_rwlock_faasm_sgx_tcs_realloc);
        free(decrypted_result);
        if ((decrypted_output = (uint8_t *) calloc(output->data_len, sizeof(uint8_t))) == NULL) {
            SET_ERROR(FAASM_SGX_OUT_OF_MEMORY);
            return 1;
        }
        if (sgx_rijndael128GCM_decrypt(&master_secret, output->data, output->data_len, decrypted_output, output->nonce, SGX_AESGCM_IV_SIZE, NULL, 0, &output->tag) != SGX_SUCCESS) {
