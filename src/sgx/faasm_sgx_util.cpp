...
extern "C" {
    ...
    faasm_sgx_status_t parse_chain_policy_to_map(std::map<const char*,std::string, cmp_str> *chain_policy_map, char* policy_str, uint32_t policy_str_len) {
        ...
        while ((token = strstr(policy_str, delim)) != NULL) {
            *token = '\0';
            ...
            if ((hash = (uint8_t *) calloc(SGX_SHA256_HASH_SIZE * 2 / 2 + 1, sizeof(uint8_t))) == NULL) {
                return FAASM_SGX_OUT_OF_MEMORY;
            }
            memcpy(key, policy_str + 1, key_len);
            hex_decode(policy_str + key_len + 2, SGX_SHA256_HASH_SIZE * 2, hash);
            chain_policy_map->insert(std::pair<char*, std::string>(key, std::string(hash, hash + 32)));
    ...
    faasm_sgx_status_t initialize_execution_stack(char **execution_stack_ptr, const uint32_t func_id) {
        ...
        strncpy(*execution_stack_ptr + strlen("<") + strlen(name) + strlen(":"), hex_hash, strlen(hex_hash));
        strncpy(*execution_stack_ptr + strlen("<") + strlen(name) + strlen(":") + strlen(hex_hash), ":", strlen(":"));
        strncpy(*execution_stack_ptr + strlen("<") + strlen(name) + strlen(":") + strlen(hex_hash) + strlen(":"), func_id_str, strlen(func_id_str));
        return FAASM_SGX_SUCCESS;
    }
    faasm_sgx_status_t push_to_execution_stack(char **execution_stack_ptr, const char* stack) {
        uint32_t new_size = strlen(*execution_stack_ptr)+ 1 + strlen(stack) + strlen(",");
        if ((*execution_stack_ptr = (char*) realloc(*execution_stack_ptr, new_size * sizeof(char))) == NULL) {
            return FAASM_SGX_OUT_OF_MEMORY; 
        }
        uint32_t temp = strlen(*execution_stack_ptr);
        strncpy(*execution_stack_ptr + temp, ",", strlen(","));
        strncpy(*execution_stack_ptr + temp + strlen(","), stack, strlen(stack));
    ...
    faasm_sgx_status_t create_total_execution_stack(char **total_stack_ptr, char *entry_stack, char *execution_stack) {
        if (entry_stack == 0x0) {
            uint32_t new_size = strlen(execution_stack) + 2;
            if ((*total_stack_ptr = (char*) calloc(new_size, sizeof(char))) == NULL) {
                return FAASM_SGX_OUT_OF_MEMORY;
            }
