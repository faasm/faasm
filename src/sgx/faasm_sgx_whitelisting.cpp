...
extern "C"
{
    ...
    faasm_sgx_status_t _register_whitelist(const char* whitelist_ptr,
                                           const char separator)
    {
        ...
        if (!(*_storage_ptr)) {
            if (!(*_storage_ptr = (_faasm_sgx_whitelist_t*)calloc(
                    sizeof(_faasm_sgx_whitelist_t) +
                      (substring_counter * sizeof(char*)),
                    sizeof(char))))
                return FAASM_SGX_OUT_OF_MEMORY;
        } else {
            _faasm_sgx_whitelist_t* tmp_ptr;
            if (!(tmp_ptr = (_faasm_sgx_whitelist_t*)realloc(
                    *_storage_ptr,
                    (sizeof(_faasm_sgx_whitelist_t) +
                     (((*_storage_ptr)->entry_len + substring_counter) *
                      sizeof(char*))) *
                      sizeof(char))))
                return FAASM_SGX_OUT_OF_MEMORY;
        ...
        for (uint32_t i = 0;; i++) {
            if (whitelist_ptr[i] == separator) {
                ...
                if (!((*_storage_ptr)
                        ->entries[(*_storage_ptr)->entry_len -
                                  substring_counter] =
                        (char*)calloc((uint32_t)(end_ptr - begin_ptr) + 1,
                                      sizeof(char)))) {
                    for (uint32_t j = 0;
                         j < ((*_storage_ptr)->entry_len - substring_counter);
                         j++)
                ...
                strncpy(
                  (*_storage_ptr)
                    ->entries[(*_storage_ptr)->entry_len - substring_counter],
                  begin_ptr,
        ...
        if (!((*_storage_ptr)->entries[(*_storage_ptr)->entry_len - 1] =
                (char*)calloc((uint32_t)(end_ptr - begin_ptr), sizeof(char)))) {
        ...
        strncpy((*_storage_ptr)->entries[(*_storage_ptr)->entry_len - 1],
                begin_ptr,
                (uint32_t)(end_ptr - begin_ptr));
    ...
    uint8_t _is_whitelisted(const char* func_name_ptr)
    {
        ...
        // If whitelist extension is activated and no whitelist is provided, all
        // native functions are blacklisted
        if (!(*_storage_ptr)) {
#if (FAASM_SGX_DEBUG)
