...
// Wrapper to call the underlying WASM runtime's memory.grow op-code to
// increase the size of the WASM's module memory
uint32_t WasmModule::growMemory(size_t nBytes)
{
    ...
    // If we can reclaim old memory, just bump the break. WASM still has not
    // implemented a mechanism to reclaim memory, so we only reclaim if Faasm
    // is managing memory instead of wasi-libc
    if (newBrk <= oldBytes) {
...
#else
    SPDLOG_TRACE("Faasm is not managing WASM memory, thus we don't mprotect-it");
#endif
