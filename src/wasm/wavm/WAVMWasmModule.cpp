...
namespace wasm {
    ...
    U32 WAVMWasmModule::getArgc() {
        return argc;
    }

    U32 WAVMWasmModule::getArgvBufferSize() {
        return argvBufferSize;
    }

    void WAVMWasmModule::prepareArgcArgv(const message::Message &msg) {
    ...
    void WAVMWasmModule::addFdForThisThread(int fd) {
        openFds.insert(fd);
    }

    void WAVMWasmModule::removeFdForThisThread(int fd) {
        openFds.erase(fd);
    }

    void WAVMWasmModule::clearFds() {
        openFds.clear();
    }

    void WAVMWasmModule::checkThreadOwnsFd(int fd) {
        const std::shared_ptr<spdlog::logger> logger = util::getLogger();
    ...
    std::string WAVMWasmModule::getBoundUser() {
        return boundUser;
    }

    std::string WAVMWasmModule::getBoundFunction() {
    ...
    void WAVMWasmModule::snapshotToFile(const std::string &filePath) {
        std::ofstream outStream(filePath, std::ios::binary);
        doSnapshot(outStream);
    }

    std::vector<uint8_t> WAVMWasmModule::snapshotToMemory() {
    ...
    size_t WAVMWasmModule::snapshotToState(const std::string &stateKey) {
    ...
    void WAVMWasmModule::restoreFromFile(const std::string &filePath) {
        // Read execution state from file
        std::ifstream inStream(filePath, std::ios::binary);
        doRestore(inStream);
    }

    void WAVMWasmModule::restoreFromMemory(const std::vector<uint8_t> &data) {
        std::istringstream inStream(std::string(reinterpret_cast<const char *>(data.data()), data.size()));
        doRestore(inStream);
    }

    void WAVMWasmModule::restoreFromState(const std::string &stateKey, size_t stateSize) {
        if (!isBound()) {
    ...
    int WAVMWasmModule::getStdoutFd() {
        if (stdoutMemFd == 0) {
    ...
    ssize_t WAVMWasmModule::captureStdout(const struct iovec *iovecs, int iovecCount) {
    ...
    ssize_t WAVMWasmModule::captureStdout(const void *buffer) {
        int memFd = getStdoutFd();
    ...
    std::string WAVMWasmModule::getCapturedStdout() {
        if (stdoutSize == 0) {
            return "";
        }
    ...
    void WAVMWasmModule::clearCapturedStdout() {
        close(stdoutMemFd);
        stdoutMemFd = 0;
        stdoutSize = 0;
    }
    ...
    storage::FileSystem &WAVMWasmModule::getFileSystem() {
        return filesystem;
    }

    wasm::WasmEnvironment &WAVMWasmModule::getWasmEnvironment() {
