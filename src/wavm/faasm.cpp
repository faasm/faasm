                               ...
                               I32 entrypointFuncArg)
{
    ...
    /*
        ...
        // Regardless if we have to individually migrate or not, we need to
       prepare
        ...
        if (funcMustMigrate) {
            ...
            std::shared_ptr<faabric::BatchExecuteRequest> req =
              faabric::util::batchExecFactory(call->user(), call->function(),
       1); req->set_type(faabric::BatchExecuteRequest::MIGRATION);

            faabric::Message& msg = req->mutable_messages()->at(0);
            msg.set_inputdata(inputData.data(), inputData.size());
            msg.set_funcptr(entrypointFuncPtr);

            // Take snapshot of function and send it to the host we are
       migrating
            // to. Note that the scheduler only pushes snapshots as part of
       function
            // chaining from the master host of the app, and
            // we are most likely migrating from a non-master host. Thus, we
       must
            // take and push the snapshot manually.
            auto* exec =
       faabric::scheduler::ExecutorContext::get()->getExecutor(); auto snap =
            ...
            faabric::util::SchedulingDecision decision(msg.appid(),
       msg.groupid()); decision.addMessage(hostToMigrateTo, msg);
            ...
            throw faabric::util::FunctionMigratedException("Migrating MPI
       rank");
        }
    */
}
