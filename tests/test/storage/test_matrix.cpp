...
void _readFileWriteState(path dummyDir,
                         const std::string& matrixPart,
                         const std::string& key)
{
    ...
    const std::vector<uint8_t> fileBytes =
      faabric::util::readFileToBytes(filePath.string());

    // Write to state
    const std::string user = getEmulatorUser();
    const std::shared_ptr<faabric::state::StateKeyValue> kv =
      faabric::state::getGlobalState().getKV(user, key, fileBytes.size());
...
TEST_CASE("Check sparse matrix file round trip", "[data]")
{
    ...
    const std::vector<uint8_t>& sizeBytes =
      faabric::util::readFileToBytes(sizePath.string());
    ...
    // Read all files in
    SparseMatrix<double> actual =
      storage::SparseMatrixFileSerialiser::readFromFiles(dummyDir.string());
...
TEST_CASE("Check sparse matrix file and state round trip", "[data]")
{
    ...
    const Map<const SparseMatrix<double>> actual =
      faasm::readSparseMatrixFromState(key.c_str(), true);
    checkSparseMatrixEquality(mat, actual);
}
}
